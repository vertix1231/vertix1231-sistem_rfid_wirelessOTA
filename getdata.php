                    		...
                    		if ($row['device_uid'] == $device_uid || $row['device_uid'] == 0){
                                ...
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                    ...
                                    if (!$row = mysqli_fetch_assoc($resultl)){
                                    	...
                                    	//INSERT INTO users_logs (id, username, serialnumber, card_uid, device_uid, device_dep, checkindate, timein, timeout, card_out) VALUES ('1','Muhamad Rizki Septiawan', 1,'24739216181','5bd9eb34be886c06','DEV1','2020-09-21','00:00:00','00:00:00','0');
                                    	
                                    	//$sql = "INSERT INTO users_logs (username, serialnumber, card_uid, device_uid, device_dep, checkindate, timein, timeout) VALUES (? ,?, ?, ?, ?, ?, ?, ?)";
                                    	$T=strval($t);
                                    	$T=preg_replace("/am/", "", $T);
                                    	$T=preg_replace("/pm/", "", $T);
                                    	$sql= "INSERT INTO users_logs (username, serialnumber, card_uid, device_uid, device_dep, checkindate, timein, timeout, card_out) VALUES ( ?, ?, ?, ? ,?, '".$d."','".$T."' ,'00:00:00','0')";
                                        $result = mysqli_stmt_init($conn);
                                        if (!mysqli_stmt_prepare($result, $sql)) {
                                            ...
                                            mysqli_stmt_bind_param($result, "sdsss", $uname, $number, $Card_uid, $Device_uid, $Device_dep);
                                            $id=$ID;
                                            $uname=$Uname;
                                            $number=$Number;
                                            $Card_uid=$card_uid;
                                    	...
                                    	$T=preg_replace("/am/", "", $T);
                                    	$T=preg_replace("/pm/", "", $T);
                                    	$sql="UPDATE users_logs SET timeout='".$T."', card_out='1' WHERE card_uid=? AND card_out='0'";
                                        $result = mysqli_stmt_init($conn);
                                        if (!mysqli_stmt_prepare($result, $sql)) {
